...
use super::super::MKS;
use super::{KubeVersion, KubeVersionsRoot};

pub fn list_kube_versions(mks: MKS) -> Result<Vec<KubeVersion>, Error> {
    let path = format!("/{}/{}", API_VERSION, KUBEVERSIONS);
    let req = mks.new_request(Method::GET, path.as_str(), None)?;
    let res = mks.do_request(req)?;

    let deserialized: KubeVersionsRoot = serde_json::from_str(res.body.as_str())
